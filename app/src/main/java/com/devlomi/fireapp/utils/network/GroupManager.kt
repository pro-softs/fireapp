...
class GroupManager {
    ...
    fun fetchAndCreatePublicGroup(groupId: String): Observable<User> {
                    ...
                    if (snapshot.exists()) {
                        for (snap in snapshot.children) {
                            ...
                            groupEvent?.let { groupEventNotNullable ->
                                ...
                                groupEvent.createGroupEventForPublicChat(groupUser, groupEvent.eventId)
    ...
    fun createPublicChatGroup(groupId: String): Completable {
        return fetchAndCreatePublicGroup(groupId).flatMapCompletable {
            return@flatMapCompletable RxFirebaseDatabase.setValue(FireConstants.groupsRef.child(groupId).child("users").child(FireManager.uid), false)
        }
    }

...
object ContactUtils {
    ...
    val users: ArrayList<PhoneContact> = ArrayList();
            ...
            try {
                ...
                while (mCursor?.moveToNext()!!) {
                    ...
                    while (!phoneNumCursor?.isAfterLast!!) {
                        ...
                        if (!phoneNumberList.contains(number)) number?.let { phoneNumberList.add(it) }
    ...
    fun syncContacts(): Completable {
        ...
        return Completable.create { emitter ->
            ...
            fetchContacts().subscribeOn(Schedulers.io()).subscribe({ user ->
                ...
                try {
                    for (user in users) {
                        ...
                        newUser.sortId = 2;
                        newUser.uid = user.phoneNumbers[0]
                        newUser.phone = user.phoneNumbers[0]
                        newUser.isHasInstalled = false;
                        newUser.isStoredInContacts = true;

                        val storedUser = realmHelper.getUser(newUser.uid)
                        if (storedUser == null) {
                            realmHelper.saveObjectToRealm(newUser)
                        } else {
                            realmHelper.updateUserInfo(newUser, storedUser, newUser.userName, newUser.isStoredInContacts)
                        }
            ...
            /*   fetchOtherContacts().subscribeOn(Schedulers.io()).subscribe({ user ->
  ...
  /*  private fun fetchOtherContacts(): Observable<User> {
        ...
        return contactsObservable.flatMap { contact ->
          ...
          return@flatMap RxFirebaseDatabase.observeSingleValueEvent(FireConstants.uidByPhone.child(formattedNumber), DataSnapshot::class.java).toObservable().map { t: DataSnapshot -> t };
                    //return@flatMap Observable.empty<Pair<PhoneContact, DataSnapshot>?>();
            //return@flatMap FireConstants.uidByPhone.child(formattedNumber).observeSingleValueEvent().toObservable().map { Pair(contact, it) }
